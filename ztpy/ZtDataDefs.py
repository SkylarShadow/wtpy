...
class ZtNpKline:
    ...
    def __init__(self, isDay:bool = False, forceCopy:bool = False):
        ...
        @forceCopy  是否强制拷贝, 如果为True, 则会拷贝一份数据, 否则会直接引用内存中的数据
                    强制拷贝主要用于ZtDtHelper的read_dsb_bars和read_dmb_bars接口, 因为这两个接口返回的数据是临时的, 调用结束就会释放
        '''
    ...
    def __getitem__(self, index:int):
        if self.__data__ is None:
            raise IndexError("No data in ZtNpKline")
    ...
    def set_data(self, firstBar, count:int):
        ...
        # 这里有点不高效，需要拼接的地方，主要是ZtDtServo的场景，这里慢点没关系
...
class ZtNpTicks:
    ...
    def __init__(self, forceCopy:bool = False):
        ...
        基于numpy.ndarray的tick数据容器
        @forceCopy  是否强制拷贝, 如果为True, 则会拷贝一份数据, 否则会直接引用内存中的数据
                    强制拷贝主要用于ZtDtHelper的read_dsb_ticks和read_dmb_ticks接口, 因为这两个接口返回的数据是临时的, 调用结束就会释放
        '''
        self.__data__:np.ndarray = None
    ...
    def __getitem__(self, index:int):
        if self.__data__ is None:
            raise IndexError("No data in ZtNpTicks")
        
        return self.__data__[index]

    def set_data(self, firstTick, count:int):
        BarList = ZTSTickStruct*count
        if self.__force_copy__:
        ...
        npAy = np.frombuffer(c_array, dtype=self.__type__, count=count)
        # 这里有点不高效，需要拼接的地方主要是ZtDtServo的场景，这里慢点没关系
        # 一旦触发拼接逻辑，都会拷贝一次
        if self.__data__ is not None:
            self.__data__ = np.concatenate((self.__data__, npAy))
            self.__data__.flags.writeable = self.__force_copy__
...
class ZtNpTransactions:
    ...
    def __init__(self, forceCopy:bool = False):
        ...
        基于numpy.ndarray的逐笔成交数据容器
        @forceCopy  是否强制拷贝, 如果为True, 则会拷贝一份数据, 否则会直接引用内存中的数据
                    强制拷贝主要用于ZtDtHelper的read_dsb_trans和read_dmb_trans接口, 因为这两个接口返回的数据是临时的, 调用结束就会释放
        '''
        self.__data__:np.ndarray = None
        self.__force_copy__:bool = forceCopy
    ...
    def __getitem__(self, index:int):
        if self.__data__ is None:
            raise IndexError("No data in ZtNpTransactions")
        
        return self.__data__[index]

    def set_data(self, firstItem, count:int):
        DataList = ZTSTransStruct*count
        if self.__force_copy__:
...
class ZtNpOrdDetails:
    ...
    def __getitem__(self, index:int):
        if self.__data__ is None:
            raise IndexError("No data in ZtNpOrdDetails")
        
        return self.__data__[index]

    def set_data(self, firstItem, count:int):
        DataList = ZTSOrdDtlStruct*count
        if self.__force_copy__:
            c_array = DataList.from_buffer_copy(DataList.from_address(addressof(firstItem.contents)))
        else:
            c_array = DataList.from_buffer(DataList.from_address(addressof(firstItem.contents)))
...
class ZtNpOrdQueues:
    ...
    def __getitem__(self, index:int):
        if self.__data__ is None:
            raise IndexError("No data in ZtNpOrdQueues")
        
        return self.__data__[index]

    def set_data(self, firstItem, count:int):
        DataList = ZTSOrdQueStruct*count
        if self.__force_copy__:
            c_array = DataList.from_buffer_copy(DataList.from_address(addressof(firstItem.contents)))
        else:
            c_array = DataList.from_buffer(DataList.from_address(addressof(firstItem.contents)))
...
class ZtBarCache:
    def __init__(self, isDay:bool = False, forceCopy:bool = False):
        self.records:ZtNpKline = None
        self.__is_day__ = isDay
        self.__force_copy__ = forceCopy
        self.__total_count__ = 0

    def on_read_bar(self, firstItem:POINTER(ZTSBarStruct), count:int, isLast:bool):
        if self.records is None:
            self.records = ZtNpKline(isDay=self.__is_day__, forceCopy=self.__force_copy__)
...
class ZtTickCache:
    def __init__(self, forceCopy:bool = False):
        self.records:ZtNpTicks = None
        self.__force_copy__ = forceCopy
        self.__total_count__ = 0

    def on_read_tick(self, firstItem:POINTER(ZTSTickStruct), count:int, isLast:bool):
        if self.records is None:
            self.records = ZtNpTicks(forceCopy=self.__force_copy__)
