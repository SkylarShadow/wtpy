...
import pandas as pd
import datetime
from ztpy.utils.check import *
from ztpy.utils.ztlog import *
from ztpy.utils.timeout import timeout
...
def get_stk_code_name_list(market: str) -> list:
    ...
    if market == MARKET.SH:
        ...
        for ind in ind_list:
            ...
            df = pd.concat([df, tmp_df]) if df is not None else tmp_df
        zt_info("获取上海证券交易所股票数量: {}", len(df) if df is not None else 0)
        return df[['code', 'name']].to_dict(orient='records') if df is not None else []
    ...
    if market == MARKET.BJ:
        ...
        zt_info("获取北京证券交易所股票数量: {}", len(df) if df is not None else 0)
        return df[['code', 'name']].to_dict(orient='records') if df is not None else []
    ...
    for ind in ind_list:
        ...
        for i in range(len(df)):
            ...
            try:
                ...
                zt_error("{}! {}", str(e), loc)
    zt_info("获取基金列表数量: {}", len(g_fund_code_name_list[market]))
