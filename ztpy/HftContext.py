...
from ztpy.ZtCoreDefs import ZTSBarStruct, ZTSOrdDtlStruct, ZTSOrdQueStruct, ZTSTickStruct, ZTSTransStruct
...
class HftContext:
    ...
    def on_getticks(self, stdCode:str, newTicks:ZtNpTicks):
        self.__tick_cache__[stdCode] = newTicks

    def on_getbars(self, stdCode:str, period:str, npBars:ZtNpKline):
        key = "%s#%s" % (stdCode, period)
        self.__bar_cache__[key] = npBars

    def on_tick(self, stdCode:str, newTick:POINTER(ZTSTickStruct)):
        '''
        tick回调事件响应
        '''
        self.__stra_info__.on_tick(self, stdCode, newTick.contents.to_dict)

    def on_order_queue(self, stdCode:str, newOrdQue:POINTER(ZTSOrdQueStruct)):
        self.__stra_info__.on_order_queue(self, stdCode, newOrdQue.contents.to_tuple())

    def on_get_order_queue(self, stdCode:str, newOdrQues:ZtNpOrdQueues):
        self.__ordque_cache__[stdCode] = newOdrQues

    def on_order_detail(self, stdCode:str, newOrdDtl:POINTER(ZTSOrdDtlStruct)):
        self.__stra_info__.on_order_detail(self, stdCode, newOrdDtl.contents.to_tuple())

    def on_get_order_detail(self, stdCode:str, newOrdDtls:ZtNpOrdDetails):
        self.__orddtl_cache__[stdCode] = newOrdDtls

    def on_transaction(self, stdCode:str, newTrans:POINTER(ZTSTransStruct)):
        self.__stra_info__.on_transaction(self, stdCode, newTrans.contents.to_tuple())

    def on_get_transaction(self, stdCode:str, newTranses:ZtNpTransactions):
        key = stdCode
    ...
    def on_bar(self, stdCode:str, period:str, newBar:POINTER(ZTSBarStruct)):
        '''
        K线闭合事件响应
        @stdCode   品种代码
        @period     K线基础周期
    ...
    def stra_get_bars(self, stdCode:str, period:str, count:int) -> ZtNpKline:
        '''
        获取历史K线
        @stdCode   合约代码
        @period K线周期,如m3/d7
    ...
    def stra_get_ticks(self, stdCode:str, count:int) -> ZtNpTicks:
        '''
        获取tick数据
        @stdCode   合约代码
        @count  要拉取的tick数量
    ...
    def stra_get_order_queue(self, stdCode:str, count:int) -> ZtNpOrdQueues:
        '''
        获取委托队列数据
        @stdCode   合约代码
        @count  要拉取的tick数量
    ...
    def stra_get_order_detail(self, stdCode:str, count:int) -> ZtNpOrdDetails:
        '''
        获取逐笔委托数据
        @stdCode   合约代码
        @count  要拉取的tick数量
    ...
    def stra_get_transaction(self, stdCode:str, count:int) -> ZtNpTransactions:
        '''
        获取逐笔成交数据
        @stdCode   合约代码
        @count  要拉取的tick数量
