...
from ztpy.ZtDataDefs import ZtNpKline, ZtNpTicks, ZtBarCache, ZtTickCache
from ztpy.wrapper.PlatformHelper import PlatformHelper as ph
from ztpy.ZtUtilDefs import singleton
import os

CB_GET_BAR = CFUNCTYPE(c_void_p,  POINTER(ZTSBarStruct), c_uint32, c_bool)
CB_GET_TICK = CFUNCTYPE(c_void_p,  POINTER(ZTSTickStruct), c_uint32, c_bool)
...
class ZtDtServoApi:
    ...
    def get_bars(self, stdCode:str, period:str, fromTime:int = None, dataCount:int = None, endTime:int = 0) -> ZtNpKline:
        '''
        获取K线数据
        @stdCode    标准合约代码
    ...
    def get_ticks(self, stdCode:str, fromTime:int = None, dataCount:int = None, endTime:int = 0) -> ZtNpTicks:
        '''
        获取tick数据
        @stdCode    标准合约代码
    ...
    def get_ticks_by_date(self, stdCode:str, iDate:int) -> ZtNpTicks:
        '''
        按天读取tick数据
        @stdCode    标准合约代码
    ...
    def get_sbars_by_date(self, stdCode:str, iSec:int, iDate:int) -> ZtNpKline:
        '''
        按天读取秒线
        @stdCode    标准合约代码
        @iSec       周期, 单位s
    ...
    def get_bars_by_date(self, stdCode:str, period:str, iDate:int) -> ZtNpKline:
        '''
        按天读取分钟线
        @stdCode    标准合约代码
        @period     周期, 分钟线
