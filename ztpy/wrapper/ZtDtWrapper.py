...
from ztpy.ZtCoreDefs import ZTSTickStruct, ZTSBarStruct, CB_PARSER_EVENT, CB_PARSER_SUBCMD, FUNC_DUMP_HISBARS, FUNC_DUMP_HISTICKS
...
class ZtDtWrapper:
    ...
    def dump_his_bars(self, id:str, fullCode:str, period:str, bars:POINTER(ZTSBarStruct), count:int) -> bool:
        id = bytes.decode(id)
        engine = self._engine
    ...
    def dump_his_ticks(self, id:str, fullCode:str, uDate:int, ticks:POINTER(ZTSTickStruct), count:int) -> bool:
        id = bytes.decode(id)
        engine = self._engine
        dumper = engine.get_extended_data_dumper(id)
        if dumper is None:
