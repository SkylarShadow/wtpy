...
from ztpy.ZtCoreDefs import CB_EXECUTER_CMD, CB_EXECUTER_INIT, CB_PARSER_EVENT, CB_PARSER_SUBCMD
from ztpy.ZtCoreDefs import CB_STRATEGY_INIT, CB_STRATEGY_TICK, CB_STRATEGY_CALC, CB_STRATEGY_BAR, CB_STRATEGY_GET_BAR, CB_STRATEGY_GET_TICK, CB_STRATEGY_GET_POSITION, CB_STRATEGY_COND_TRIGGER
from ztpy.ZtCoreDefs import EVENT_PARSER_CONNECT, EVENT_PARSER_DISCONNECT, EVENT_PARSER_INIT, EVENT_PARSER_RELEASE
from ztpy.ZtCoreDefs import CB_HFTSTRA_CHNL_EVT, CB_HFTSTRA_ENTRUST, CB_HFTSTRA_ORD, CB_HFTSTRA_TRD, CB_SESSION_EVENT, CB_HFTSTRA_POSITION
from ztpy.ZtCoreDefs import CB_HFTSTRA_ORDQUE, CB_HFTSTRA_ORDDTL, CB_HFTSTRA_TRANS, CB_HFTSTRA_GET_ORDQUE, CB_HFTSTRA_GET_ORDDTL, CB_HFTSTRA_GET_TRANS
from ztpy.ZtCoreDefs import CHNL_EVENT_READY, CHNL_EVENT_LOST, CB_ENGINE_EVENT
from ztpy.ZtCoreDefs import FUNC_LOAD_HISBARS, FUNC_LOAD_HISTICKS, FUNC_LOAD_ADJFACTS
from ztpy.ZtCoreDefs import EVENT_ENGINE_INIT, EVENT_SESSION_BEGIN, EVENT_SESSION_END, EVENT_ENGINE_SCHDL
from ztpy.ZtCoreDefs import ZTSTickStruct, ZTSBarStruct, ZTSOrdQueStruct, ZTSOrdDtlStruct, ZTSTransStruct
from ztpy.ZtDataDefs import ZtNpKline, ZtNpOrdDetails, ZtNpOrdQueues, ZtNpTicks, ZtNpTransactions
...
class ZtWrapper:
    ...
    def on_stra_tick(self, id:int, stdCode:str, newTick:POINTER(ZTSTickStruct)):
        engine = self._engine
        ctx = engine.get_context(id)
    ...
    def on_stra_bar(self, id:int, stdCode:str, period:str, newBar:POINTER(ZTSBarStruct)):
        engine = self._engine
        ctx = engine.get_context(id)
        if ctx is not None:
    ...
    def on_stra_get_bar(self, id:int, stdCode:str, period:str, curBar:POINTER(ZTSBarStruct), count:int, isLast:bool):
        '''
        ...
        npBars = ZtNpKline(isDay, forceCopy=False)
        npBars.set_data(curBar, count)

        if ctx is not None:
            ctx.on_getbars(bytes.decode(stdCode), period, npBars)

    def on_stra_get_tick(self, id:int, stdCode:str, curTick:POINTER(ZTSTickStruct), count:int, isLast:bool):
        '''
        ...
        npTicks = ZtNpTicks(forceCopy=False)
    ...
    def on_hftstra_order_queue(self, id:int, stdCode:str, newOrdQue:POINTER(ZTSOrdQueStruct)):
        stdCode = bytes.decode(stdCode)
        engine = self._engine
        ctx = engine.get_context(id)
    ...
    def on_hftstra_get_order_queue(self, id:int, stdCode:str, newOrdQue:POINTER(ZTSOrdQueStruct), count:int, isLast:bool):
        engine = self._engine
        ctx = engine.get_context(id)
        npHftData = ZtNpOrdQueues(forceCopy=False)
        npHftData.set_data(newOrdQue, count)

        if ctx is not None:
            ctx.on_get_order_queue(bytes.decode(stdCode), npHftData)

    def on_hftstra_order_detail(self, id:int, stdCode:str, newOrdDtl:POINTER(ZTSOrdDtlStruct)):
        engine = self._engine
        ctx = engine.get_context(id)
    ...
    def on_hftstra_get_order_detail(self, id:int, stdCode:str, newOrdDtl:POINTER(ZTSOrdDtlStruct), count:int, isLast:bool):
        engine = self._engine
        ctx = engine.get_context(id)
        
        npHftData = ZtNpOrdDetails(forceCopy=False)
        npHftData.set_data(newOrdDtl, count)
            
        if ctx is not None:
            ctx.on_get_order_detail(bytes.decode(stdCode), npHftData)

    def on_hftstra_transaction(self, id:int, stdCode:str, newTrans:POINTER(ZTSTransStruct)):
        engine = self._engine
        ctx = engine.get_context(id)
    ...
    def on_hftstra_get_transaction(self, d:int, stdCode:str, newTrans:POINTER(ZTSTransStruct), count:int, isLast:bool):
        engine = self._engine
        ctx = engine.get_context(id)
        
        npHftData = ZtNpTransactions(forceCopy=False)
    ...
    def on_load_fnl_his_bars(self, stdCode:str, period:str):
        ...
        # feed_raw_bars(ZTSBarStruct* bars, ZtUInt32 count);
    ...
    def feed_adj_factors(self, stdCode:str, dates:list, factors:list):
        ...
        feed_adj_factors(ZtString stdCode, ZtUInt32* dates, double* factors, ZtUInt32 count)
        '''
    ...
    def on_load_his_ticks(self, stdCode:str, uDate:int):
        ...
        # feed_raw_ticks(ZTSTickStruct* ticks, ZtUInt32 count);
    ...
    def push_quote_from_exetended_parser(self, id:str, newTick:POINTER(ZTSTickStruct), uProcFlag:int = 1):
