...
class ZtBtTask(BtEventSink):
    '''
    回测任务类
    '''
    def __init__(self, user:str, straid:str, btid:str, folder:str, logger:ZtLogger = None, sink:BtTaskSink = None):
        self.user = user
        self.straid = straid
        self.btid = btid
        self.logger = logger
        ...
        self._cmd_line = None
        self._mq_url = "ipc:///ztpy/bt_%s.ipc" % (btid)
        self._ticks = 0
        self._state = 0
        self._procid = None
...
class ZtBtMon(BtTaskSink):
    '''
    回测管理器
    '''
    def __init__(self, deploy_folder:str, dtServo:ZtDtServo = None, logger:ZtLogger = None):
    ...
    def run_backtest(self, user:str, straid:str, fromTime:int, endTime:int, capital:float, slippage:int=0) -> dict:
        ...
        # 添加
        btTask = ZtBtTask(user, straid, btid, folder, self.logger, sink=self)
        btTask.run()
