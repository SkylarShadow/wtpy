...
def get_path_tree(root: str, name: str, hasFile: bool = True):
    ...
    for filename in files:
        if filename in ['__pycache__', '.vscode', 'ztpy', '__init__.py']:
            continue
        if filename[-3:] == 'pyc':
            continue
...
class ZtMonSvr(WatcherSink):

    def __init__(self, static_folder: str = "static/", deploy_dir="C:/", sink: ZtMonSink = None, notifyTimeout:bool = True):
        '''
        ZtMonSvr构造函数
        ...
        self.logger = ZtLogger(__name__, "ZtMonSvr.log")
        self._sink_ = sink

        # 数据管理器，主要用于缓存各组合的数据
        self.__data_mgr__ = DataMgr('data.db', logger=self.logger)

        self.__bt_mon__: ZtBtMon = None
        self.__dt_servo__: ZtDtServo = None
        ...
        app = FastAPI(title="ZtMonSvr", description="A http api of ZtMonSvr", redoc_url=None, version="1.0.0")
        app.add_middleware(GZipMiddleware, minimum_size=1000)
        app.add_middleware(SessionMiddleware, secret_key='!@#$%^&*()', max_age=25200, session_cookie='ZtMonSvr_sid')
        app.add_middleware(
            CORSMiddleware,
            allow_origins=["*"],
    ...
    def enable_token(self, seckey: str = "ZtMonSvr@2021"):
        '''
    ...
    def set_bt_mon(self, btMon: ZtBtMon):
        '''
        设置回测管理器

        @btMon      回测管理器ZtBtMon实例
        '''
        self.__bt_mon__ = btMon
        self.init_bt_apis(self.app)

    def set_dt_servo(self, dtServo: ZtDtServo):
        '''
        设置DtServo

        @dtServo    本地数据伺服ZtDtServo实例
        '''
    ...
    def init_mgr_apis(self, app: FastAPI):
        ...
        async def qry_mon_logs(
            ...
            filename = os.getcwd() + "/logs/ZtMonSvr.log"
